...
train_set <- loan_data_clean[train_index, ]
...
train_set_replace <- train_set
test_set_replace <- test_set
train_set_replace$default <- ifelse(train_set_replace$default == "yes", 1, 0)
set.seed(123)
gbm_model <- gbm(formula = default~., distribution = "bernoulli",
                    data = train_set_replace,
                    n.trees = 10000)
summary(gbm_model)
test_set_replace$default <- ifelse(test_set_replace$default == "yes", 1, 0)
pred_gbm <- predict(gbm_model, 
                    newdata = test_set_replace,
                    n.trees = 10000,
                    type = "response")
range(pred_gbm)
...
auc_gbm <- auc(test_set_replace$default, pred_gbm)
auc_oob_gbm <- auc(test_set_replace$default, pred_oob_gbm)
auc_cv_gbm <- auc(test_set_replace$default, pred_cv_gbm)
print(paste0("Test set AUC (GBM): ", auc_gbm))
print(paste0("Test set AUC (OOB): ", auc_oob_gbm))                         
print(paste0("Test set AUC (CV): ", auc_cv_gbm))

#== Rank all models by AUC measures (lowest to highest) ==#
auc_list <- data.frame(Models=c("Logit_full", "Logit Regression", "Probit Regression",
...
rank_models <- auc_list[sort.list(auc_list$AUC), "Models"]
print(rank_models)

preds_list <- list(pred_full, pred_logit, pred_probit, 
                   pred_rf, pred_rf_tune, pred_gbm, pred_oob_gbm, pred_cv_gbm)
m <- length(preds_list)
actuals_list <- rep(list(test_set_replace$default), m)
