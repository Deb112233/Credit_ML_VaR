...
write.table(sum_fv, file = "C:/Users/siqiwen/Desktop/Credit/sum_table.csv",  row.names=FALSE, sep = ",",  qmethod = "double")
rm(list = ls(pattern = "sum"))
#==== Exploratory relationships between variables ===#
library(lattice)
...
bwplot(int_rate~purpose+dependents|default, xlab = "Loan Purpose & No. of Dependents", ylab = "Interest Rate")
bwplot(int_rate~housing+credit_history|default, xlab = "Ownership & Credit History", ylab = "Interest Rate")
...
loan_data_replace$emp_cat[which(loan_data_replace$emp_length > 45)] <- "45+"
